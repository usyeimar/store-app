...
class JoinExpensesCategoriesModel extends Model{
    ...
    public function getAll($userid){
        $items = [];
        try{
            $query = $this->prepare('SELECT tbexpenses.id as expense_id, title, category_id, amount, date, id_user, tbcategories.id, name, color  FROM tbexpenses INNER JOIN categories WHERE tbexpenses.category_id = tbcategories.id AND tbexpenses.id_user = :userid ORDER BY date');
    ...
    function getTotalByMonthAndCategory($date, $categoryid, $userid){
        try{
            ...
            $query = $this->db->connect()->prepare('SELECT SUM(amount) AS total from tbexpenses WHERE category_id = :val AND id_user = :user AND YEAR(date) = :year AND MONTH(date) = :month');
